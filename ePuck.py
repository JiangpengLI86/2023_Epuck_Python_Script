...
class ePuck():
    ...
    def _debug(self, *txt):
        ...
        if self.debug:
            print('\033[31m[ePuck]:\033[0m ', ' '.join([str(e) for e in txt]), file=sys.stderr)
    ...
    def _read_image(self):
        ...
        try:
            n = self._send(msg)
            self._debug("Reading Image: sending " + repr(msg) + " and " + str
                (n) + " bytes")
    ...
    def _write_actuators(self):
        ...
        for m in actuators:
            if m[0] == 'L':
                ...
                n = self._send(msg)
                self._debug \
                    ('Binary message sent of [' + str(n) + '] bytes: ' + str
                        (struct.unpack('<bbb', msg)))
    ...
    def disable(self, *sensors):
        ...
        for sensor in sensors:
            try:
                ...
                self._debug \
                    ('Something wrong happened to disable the sensors: ', e)
